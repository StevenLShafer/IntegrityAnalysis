...
app_server <- function(input, output, session) {
  ...
  output$issueLegend <- renderUI({
    ...
    div(style = "margin: 4px 0 8px 0; font-size: 13px;",
        ...
        if (hasFailsafe) entry("#ffd8a8", "fail-safe count", paste(
          ...
          "this arm size. Every whole table the printed percentages allow",
          "is built - each cell inside its own bracket, each arm's counts",
          "adding up to that arm's N - and scored the way the analysis",
          "scores it; the one used is the one with the LARGEST p, so the",
          "authors get every benefit of the doubt the page allows. A",
          "design decision for incomplete data: the printed counts would",
          "settle it. Hover the cell for the bracket and for the best and",
          "worst p the page could have produced.")),
  ...
  output$dataGrid <- rhandsontable::renderRHandsontable({
    ...
    paintJS <- paste0(
      ...
      "                counts for this arm size. Of every whole table the ",
      "                printed percentages allow - each arm adding up to ",
      "                its own N - the reading used is the one with the ",
      "                LARGEST p, the best case for the authors. Hover ",
      "                shows the bracket and the best and worst p; the ",
      "                printed counts would settle it.',",
