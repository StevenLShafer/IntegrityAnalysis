...
app_server <- function(input, output, session) {
  ...
  output$issueLegend <- renderUI({
    ...
    div(style = "margin: 4px 0 8px 0; font-size: 13px;",
        ...
        if (hasFailsafe) entry("#ffd8a8", "fail-safe count", paste(
          ...
          "this arm size; of every set of counts the printed percentages",
          "allow, the row is built from the one that leaves the arms",
  ...
  output$dataGrid <- rhandsontable::renderRHandsontable({
    ...
    paintJS <- paste0(
      ...
      "                counts for this arm size; of every set of counts ",
      "                the printed percentages allow, the row is built ",
      "                from the one that leaves the arms least alike ",
