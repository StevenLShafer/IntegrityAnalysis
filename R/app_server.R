...
app_server <- function(input, output, session) {
  ...
  output$issueLegend <- renderUI({
    ...
    div(style = "margin: 4px 0 8px 0; font-size: 13px;",
        ...
        if (hasFailsafe) entry("#ffd8a8", "fail-safe count", paste(
          ...
          "is built - each cell inside its own bracket - and scored the",
          "way the analysis scores it; the one used is the one with the",
          "LARGEST p, so the authors get every benefit of the doubt the",
          "page allows. Where the page allows more readings than can be",
          "counted, nothing is filled in: the cells stay blank and the",
          "row is not analysed. A design decision for incomplete data:",
          "the printed counts would settle it. Hover the cell for the",
          "bracket and for the best and worst p the page could have",
          "produced.")),
  ...
  output$dataGrid <- rhandsontable::renderRHandsontable({
    ...
    paintJS <- paste0(
      ...
      "                counts for this arm size. Every whole table the ",
      "                printed percentages allow is enumerated, and the ",
      "                reading used is the one with the LARGEST p, the ",
      "                best case for the authors. If there are too many ",
