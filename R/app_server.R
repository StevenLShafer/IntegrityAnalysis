...
app_server <- function(input, output, session) {
  ...
  output$issueLegend <- renderUI({
    ...
    div(style = "margin: 4px 0 8px 0; font-size: 13px;",
        ...
        if (hasFailsafe) entry("#ffd8a8", "fail-safe count", paste(
          ...
          "this arm size; the ambiguous cells are split against each",
          "other, each taking the end of its bracket that leaves the arms",
          "least alike, so the row can look less alike than the truth but",
          "never more. A design decision for incomplete data: the analysis",
          "is conservative here - a row that would alarm on printed counts",
          "usually will not alarm on printed percentages - and the printed",
          "counts would settle it. Hover the cell for the bracket.")),
  ...
  output$dataGrid <- rhandsontable::renderRHandsontable({
    ...
    paintJS <- paste0(
      ...
      "                counts for this arm size; the ambiguous cells are ",
      "                split against each other, each taking the end of ",
      "                its bracket that leaves the arms least alike ",
      "                (fail-safe: the row can look less alike than the ",
      "                truth, never more). Hover shows the bracket; the ",
