...
# a two-arm percent-only table as a synthetic PDF (the fixture style of
# test-derived-green.R), through the deterministic reader
pctPdf <- function(pctA, pctB, nA, nB) {
  ...
  cells <- c(
    ...
    rowCells(136, "", c(sprintf("(n = %d)", nA), sprintf("(n = %d)", nB)), vx),
    rowCells(164, "Age (yr)", c("45.3 ± 12.1", "46.1 ± 11.8"), vx),
    rowCells(190, "Male sex", c(pctA, pctB), vx))
...
test_that("two arms printing the same ambiguous percentage are pushed APART, and the cells carry the fail-safe kind", {
  r <- parseBaselineTableHeuristics(pctPdf("50%", "50%", 1000, 1000), pctApprox = TRUE, quiet = TRUE)
  ...
  r2 <- parseBaselineTableHeuristics(pctPdf("50%", "52%", 40, 25), pctApprox = TRUE, quiet = TRUE)
  expect_false(any(r2$derivedCells$KIND == "failsafe"))
})

test_that("the fail-safe rule itself: arms above the pooled proportion take the top of their bracket, below take the bottom, ties alternate", {
  # replicate the choice in isolation, as the heuristics apply it
  choose <- function(lo, hi, cnt, N) {
    approx <- is.na(cnt)
    mid <- ifelse(approx, (lo + hi) / 2, cnt)
    pooled <- sum(mid) / sum(N)
    side <- sign(mid / N - pooled)
    tied <- which(approx & side == 0)
    if (length(tied)) side[tied] <- rep(c(-1, 1), length.out = length(tied))
    ifelse(approx, ifelse(side > 0, hi, lo), cnt)
  }
