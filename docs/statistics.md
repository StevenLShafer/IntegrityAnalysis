...
| Column | Meaning |
|---|---|
| P | The one-sided p toward homogeneity. "<0.0001" means the 97.5% upper confidence bound clears 0.0001. Text entries ("Only 1 Row", "Quartiles do not increase (Q3 must exceed Q1)" — printed the wrong way round, since quartiles that merely print the same value are analyzed, "Printed precision beyond this magnitude's numerical resolution", "The stated precision does not match the printed values", ...) are refusals: the row could not be analyzed, with the reason. |
...
- **A stated precision describes the number beside it.** The precision
  columns say what grid a printed value sits on, and the engine turns
  that grid into an interval: a printed SD stands for half a step either
  side, and so does a printed quartile. A grid the value does not sit on
