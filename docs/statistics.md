...
| Note | On the Summary line, "k of n rows analysed" when the engine refused any row of the trial, or the validator left one out — a row with a label and no values (a percentage block the parser could not reconstruct arrives that way), or a categorical line with no partner arm; such a row is listed with P "Not analysed" and the reason in its note — so a combined p is never read as covering a table it did not cover. On a variable's line: "attainable floor" when the arms agree exactly and no honest replicate agreed better (see "Rounding, convergence, and the attainable floor"); for a median row, "quartiles beyond the metalog's skew limit; fitted at the limit" when the printed quartiles imply more skew than a three-term metalog can carry, so the closest feasible one was used, and "printed quartiles do not separate in k arm(s); the fit uses their printed intervals" when an arm prints Q1 and Q3 as the same value, so its width is known only to be smaller than one printed unit; "the stated mean precision (k decimals) exceeds the digits these values carry, and the arms are equal at the digits they print, so the p rests on that precision - check it against the page" when a row whose arms print the same mean states more decimals than those means carry; and "the stated mean precision (k decimals) is coarser than the digits these values carry, which widens the rounding the arms are judged against; this row's p rests on that claim - check it against the page" for the mirror case. Notes are joined with "; ". Blank otherwise. |
...
- **A stated precision describes the number beside it.** All three
  precision columns say what grid a printed value sits on, and the engine
  turns that grid into an interval: a printed SD stands for half a step
  either side, so does a printed quartile, and the observation precision
  ...
  already held to the median's lattice by the rule above. Coarser than the printed digits is not refused either, and for the same
  reason — a paper printing "50" may honestly have rounded to tens — but
  it is disclosed, because it moves the p the other way: it widens the
  rounding the arms are judged against and can take a row from the
  reportable floor to unremarkable. Every guard in this list points at the
  accusing direction; this note is the one that points at the excusing
  one. Too fine is likewise not refused but
  ...
  The check is arithmetic, not a matter of taste: quartiles of 40 and 60
  reported to the nearest ten are consistent and are analyzed, and so is
  a variable whose interquartile range is smaller than one printed unit.
  It matters because the interval is a direct multiplier on the null's
  spread: before the check, a single spreadsheet cell claiming five
