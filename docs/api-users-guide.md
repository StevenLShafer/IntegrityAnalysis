...
| CSV bytes | a gzip, bzip2 or xz stream named `.csv` is refused by its first bytes, before any reader opens it (R would inflate it transparently; the request cap bounds the compressed bytes only) | 422 with the reason (`stage: "parse"` from `/analyze`) |
