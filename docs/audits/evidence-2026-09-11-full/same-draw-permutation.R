...
ans <- list()
for(J in c(9,14)) for(tag in c('baseline','flip-extreme')) {
  id <- paste0('J',J,'-',tag,'-seed42')
  .auditInputs <- list()
  ...
  write.csv(do.call(rbind,ans),file.path(out,'same-draw-permutation.csv'),row.names=FALSE)
  cat('DONE',id,'keys',length(unique(.auditFinal$keyOf)),'ties',ties,'\n');flush.console()
}
untrace('.iaTieCounts',where=ns);untrace('P_Calc',where=ns)
writeLines('Complete',file.path(out,'same-draw-permutation-complete.txt'))
