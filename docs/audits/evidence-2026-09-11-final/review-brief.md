# Independent statistical audit of IntegrityAnalysis — brief for the second FULL pass

Written 2026-09-10 (night) for the audit Steve Shafer will commission
next. Your previous pass (the full audit of 2026-09-10, committed
verbatim at `docs/audits/2026-09-10-full-independent-statistical-audit-chatgpt.md`
with its 272 evidence files) re-verified every earlier finding and
found three new things: F1 (rows with identical null laws mapped
separately, splitting a genuine trial tie), F2 (a trial whose every row
is excluded vanished), F3 (`row: "NA"` where the contract says null).
All three are fixed, with your CSVs as the tests, and the security
screen of the F1 merge found and fixed one thing in the fix itself
(the shared mapping was quadratic in the rows sharing a law). A
private security audit ran in parallel; its five findings are fixed
too, and two of them touch what the statistics receive (below).

This brief asks the same question as the last one: **a full audit of
the engine as it stands.** Steve's standard is unchanged — "better is
the enemy of done". If this pass finds no P1 and no numerical P2, the
engine is declared done and the effort moves to corpus validation.
...
Audit `main` at commit **`7fd6545`** (2026-09-10, 22:45 PDT), the tip
after the last fix merged. Check that commit out. Production runs it
(the app and the API both report it). The whole suite (about 3,600
assertions, the real-HTTP tests on) and the static tripwire pass on
it. Every statistical change is in commits at or before `84051f3`;
the twenty-odd commits after it are the spreadsheet and CSV readers
(bounds on their work and memory, found by six security screens in a
row) and touch nothing the engine computes - the CSV template route
your cases use is unchanged in what it reads, only in what it refuses
(a line over 100 KB, a cell over 2,000 characters, a compressed stream
named `.csv`). If `main` has moved on by the time you start, the brief
still names `7fd6545`.
...
`7fd6545`, and report VERIFIED STILL FIXED / REGRESSED / COULD NOT TEST
...
shared mapping — in particular whether pooling changes any p for rows
whose laws are *not* identical (it must not: a row with a unique key is
mapped exactly as before), and whether a pooled mapping over G rows'
draws and a single row's own mapping agree in expectation. Anything
...
## What changed since your last pass (`a796e51` → `7fd6545`)

- **Your F1.** `.iaNullKey()` names each row's null law (continuous
  and median rows by their parameters and printed steps; categorical
  rows by their margins). Rows sharing a key are mapped through one
  pooled empirical distribution; draws are made in the original row
  order so the RNG stream is unchanged; the trial tie is bounded. Then
  the screen's fix: only shared rows' draws are held, the pool is
  sorted once, each row is counted by two `findInterval` searches, and
  a trial needing more than 10,000,000 held draws at the final stage is
  refused before a draw (a hundred identical rows at the 100,000
  ceiling are within the bound; the app and the API both refuse with
  the reason).
- **Your F2.** `validateData()` builds the trial list from every trial
  offered, before exclusion; `P_Calc()` on a trial with no analysable
  row prints its excluded lines and a Summary of "No values", "0 of n
  rows analysed"; `sumz()` skips it.
- **Your F3.** The handler turns a typed NA into a logical NA before
  serialisation; `row` is null on the wire.
- **The read-across.** The three sentences you named.
- **Security S1, S2** (above): what the engine receives from a refused
  resubmission and from the AI converter.
- **Security S3–S5**: no statistical content (console echo default,
  a multipart part not named `file`, the ten-sheet refusal on the
  template fallback).
...
| item | state |
|---|---|
| Your F1, F2, F3 of the full pass | **fixed** — verify, as regression items in the checklist |
...
- Your evidence directory will be committed verbatim with the report,
  as the last five were. Keep machine-local paths out of it.
