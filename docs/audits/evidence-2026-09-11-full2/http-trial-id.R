...
tryCatch({
  ...
  for(endpoint in c('parse','analyze'))for(id in c('trial-id-200-delta-0.5','trial-id-201-delta-0.5','trial-id-template-201')){
    ...
    write.csv(do.call(rbind,ans),file.path(out,'http-trial-id-summary.csv'),row.names=FALSE)
    cat('DONE',endpoint,id,'\n');flush.console()
  }
},finally={px$kill()})
stopifnot(!px$is_alive())
writeLines('Complete; local service stopped.',file.path(out,'http-trial-id-complete.txt'))
