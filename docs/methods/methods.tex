...
\end{equation}
This normal reference is exact when independent trial inputs are uniform under the null; independence and continuity alone are insufficient. The actual trial inputs are generally discrete mid-p estimates under fitted models, so across-trial calibration is an additional approximation not repaired by the within-trial simulation. Since 25 September 2026 the implementation combines the trials' \emph{numerical} Monte Carlo estimates---the floored mid-p, $1/(M+1)$ at the smallest---rather than their displayed strings; a results table that carries no numbers (one from an earlier build, or a value typed by hand) falls back to the display, ``$<0.0001$'' read as $10^{-4}$. Two trials each at the floor for $M = 10^{5}$ therefore combine to $8.1 \times 10^{-10}$, where their displayed values would give $7.2 \times 10^{-8}$; the floor is the resolution at which the simulation stops, not a calibrated tail probability, so the fineness of such a combined value reflects the floor rule rather than established resolution. Trials without a computable result are omitted; their coverage must remain visible. No overall Monte Carlo interval is supplied. The trial set and handling of duplicate publications should be specified before inspecting results. Stable trial identity is also essential; the third audit found a reader bound merging two trials, fixed the same day (Section~\ref{sec:buildstatus}).
