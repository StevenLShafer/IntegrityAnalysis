...
> after-treatment columns. The app recognises this layout and reads **the
> Baseline column only**; the after-treatment values are a drug effect, not
> a random sample of one population, and are deliberately left out. Such
...
- **Lavender — warning.** The table passed validation, but the value
  ...
  (fixed by design or by a floor, not a sample), a row printing the same
  mean and SD in every arm (check the page for a column copied across
  arms or a duplicated variable; if the arms are separate samples the
  row stays in — agreement this close is what the screen weighs), or a
  row identical in N, mean and SD to another row of the same trial (a
  duplicate?). The
